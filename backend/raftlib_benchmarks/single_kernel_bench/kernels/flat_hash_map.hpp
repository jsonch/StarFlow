...
#include "meas.h"
#include "perfCounterInterface.h"
...
public:
    ...
    // template<typename Key, typename... Args>
    // std::pair<iterator, bool> emplace_meas(Key && key, Args &&... args)
    // {   
    //     uint64_t sTsc, fTsc;
    //     // Measure t_hash -- cycles to calculate hash of flow.
    //     sTsc = rdtscp();
    ...
    //     fTsc = rdtscp();
    //     recordtHash((long long) fTsc - sTsc);

    //     // Measure tLookup -- cycles to find and return entry or insert and return new entry.
    //     int8_t distance_from_desired = 0;
    //     sTsc = rdtscp();
    //     for (; current_entry->distance_from_desired >= distance_from_desired; ++current_entry, ++distance_from_desired)
    //     {
    //         if (compares_equal(key, current_entry->value)) {
    //             fTsc = rdtscp();
    //             recordtLookup((long long) fTsc - sTsc);
    //             return { { current_entry }, false };
    //         }
    //     }
    //     SKA_NOINLINE(std::pair<iterator, bool>) empNewRes = emplace_new_key(distance_from_desired, current_entry, std::forward<Key>(key), std::forward<Args>(args)...);
    //     fTsc = rdtscp();
    //     recordtLookup((long long) fTsc - sTsc);
    //     return empNewRes;
    // }
    ...
    template<typename Key, typename... Args>
    std::pair<iterator, bool> emplace_ptr(void * ptr, Key && key, Args &&... args)
    {   
        EntryPointer current_entry = (EntryPointer) ptr;

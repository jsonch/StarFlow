...
// 			    mmappedData = mmap(NULL, f_sz, PROT_READ, FLAGS, 0, 0);
// 			    if (mmappedData == MAP_FAILED) {
// 					perror("mmap");
// 					exit(1);
// 				}
// 			    // assert(mmappedData != MAP_FAILED);
// 			    // int rc = madvise(mmappedData, f_sz, MADV_SEQUENTIAL);
// 			    // if (rc != 0)
// 			    //     perror("madvise");
// 			    // assert(rc == 0);
// 			    inBuf = (char *) mmappedData;
// 				fread((char *)inBuf, f_sz, 1, f);
// 				fclose(f);

// 				bufPos = inBuf;
// 				bufEnd = inBuf + f_sz;
// 				std::cout << "ret from main" << endl;
// 			}
			...
			MicroflowReader(const std::string& fn) : raft::kernel()
			{
				...
				insz.close();
				// std::cout << "looping " << totalMfCt << " mCLFRs" << endl;
			...
			void sendBatch(){
				finalClfrCt+=batchVec->size();
					...
					for (auto kv : (*batchVec)){
						o.write(kv.first.c_str(), 13);
						// if (kv.second.flowFeatures.pktCt > 1000){
						// 	std::cout << "writing pkt ct: " << kv.second.flowFeatures.pktCt << std::endl;
						// }

...
#ifndef STARFLOW_KERNELS_FEATURE_CALCULATOR_PFE
#define STARFLOW_KERNELS_FEATURE_CALCULATOR_PFE
...
namespace starflow {
	namespace kernels {
		template<typename T>
		class PFEFeatureCalculator : public raft::kernel
		{
		public:
			...
			void getFeatures(std::pair<std::string, CLFR_Value> &CLFR_Pair){
				...
				// Compute 1 features of each vec: minimum, maximum, mean, std dev
				// std::cout << "computing features" << std::endl;
				computeStats(0, interArrivals);
				computeStats(1, packetLens);
				...
				curFeatures(2)=lsum;
				// total duration.
				curFeatures(3)=float(CLFR_Pair.second.packetVector[vecLen-1].ts)-float(CLFR_Pair.second.packetVector[0].ts);
				// total pkt ct.
				curFeatures(4)=float(vecLen);

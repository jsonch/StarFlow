...
// Extracts complex features for application prediction.
// (example 2)
	...
	namespace kernels {
		...
		public:
			...
			void getFeatures(std::pair<std::string, CLFR_Value> &CLFR_Pair){
				...
				// curFeatures(12+MINPKT_CT)=lsum;
				// // total duration.
				// curFeatures(12+MINPKT_CT+1)=float(CLFR_Pair.second.packetVector[vecLen-1].ts)-float(CLFR_Pair.second.packetVector[0].ts);
				// // total pkt ct.
				// curFeatures(12+MINPKT_CT+3)=float(vecLen);

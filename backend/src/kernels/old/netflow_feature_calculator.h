...
#ifndef STARFLOW_KERNELS_FEATURE_CALCULATOR_NETFLOW
#define STARFLOW_KERNELS_FEATURE_CALCULATOR_NETFLOW
...
namespace starflow {
	namespace kernels {
		template<typename T>
		class NetFlowFeatureCalculator : public raft::kernel
		{
		public:
			// features for classifier. -- just 3 features -- pkt ct, byte ct, duration.
			...
			explicit NetFlowFeatureCalculator()
				: raft::kernel()
			{
			...
			void getFeatures(std::pair<std::string, CLFR_Value> &CLFR_Pair){
				...
				curFeatures(0)=lsum;
				// total duration.
				curFeatures(1)=float(CLFR_Pair.second.packetVector[vecLen-1].ts)-float(CLFR_Pair.second.packetVector[0].ts);
				// total pkt ct.
				curFeatures(2)=float(vecLen);

...
#define BENCHMARK_CT_MICROBURST 100000
	...
	namespace kernels {
		...
		public:
				...
				: raft::kernel()
			{
				...
				output.template addPort<std::unordered_set<uint32_t>>("out");
			...
			raft::kstatus run() override
			{
				...
				for (auto kv : clfrVector){
					pktCounter += kv.second.packetVector.size();
					// Insert into cache. 
					...
					// if above threshold, and queue is growing.
						if (localMaxSize >= queueThreshold){
							...
							// }
							// cout << "] )" << endl;
							// Report the set of unique offenders.
							auto &out( output["out"].template allocate<std::unordered_set<uint32_t>>() );
							std::unordered_set<uint32_t> uniqueOffenders = out;
							for (auto h : packetSrcsInQueue){
								uniqueOffenders.insert(h);
							}
							// cout << "occupants["<<kv.second.packetVector[0].ts<<"] = (" <<pctAboveThreshold << ", [ ";
				...
				if (pktCounter > BENCHMARK_CT_MICROBURST) {
					// calculate and report throughput in packets per second.
